...
outvid_name = "{}.mp4".format(ts.strftime("%Y-%m-%d_%H-%M-%S"))
...
# IMAGE PROCESSING:
            ...
            #
            BstartX, BstartY, BendX, BendY, max_size, Bconfidence = [0, 0, 0, 0, 0, 0] # init value 
            ...
            cv2.rectangle(frame, (BstartX, BstartY), (BendX, BendY),(0, 0, 255), 2) #draw the bounding box
            cv2.circle(frame, (int(BstartX + (BendX-BstartX)/2), BstartY + int((BendY-BstartY)/2)), 10, (0, 255, 0), 1) #draw the center of bb
            area = (BendX - BstartX)*(BendY - BstartY) #calculate the area of boudning box 
            text = "{:.2f}%".format(Bconfidence * 100) # confidence
            cv2.putText(frame, text, (BstartX, y),cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2) #print the confidence next to the bb

            #calculate the distance between the center of frame and the center of the detected face 
            diff_x = w/2 - (BstartX + (BendX-BstartX)/2)
            diff_y = h/2 - (BstartY + (BendY-BstartY)/2)
            abs_diff = math.sqrt(diff_x**2 + diff_y**2) 

            diff_area = 20000 - area #20000 is a standard frame size for a decent distance
            cv2.putText(frame,'%d, %d, %d cm' %(diff_x/ref_w_px*ref_width, diff_y/ref_w_px*ref_width, diff_area/ref_area*ref_dist), (0,20), cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,255),3,cv2.LINE_AA)
            out.write(frame)
            ...
            pt = ct
            K = 0.0026 # fine-tuned coeefficient
            if Override == False:
                if BstartX == 0 and BstartY == 0: 	
                    ...
                    #set the upper bound and lower bound for the velocity so that the drone does not move abruptly 
...
def init_face_recon():
    ...
    global out 
    out = cv2.VideoWriter(outvid_file, fourcc, FPS, (960, 720)) #25 fps and 960x720 frame
    time.sleep(2.0)
